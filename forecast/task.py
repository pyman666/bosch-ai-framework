...
Forecast 等业务复用同一份任务表 (单进程内存 dict), 任务被划分为若干 **phase**
(自由字符串, e.g. ``"forecast" / "evaluate"``), 客户端按 phase 轮询拿结果.
多 worker 部署时任务表不共享, 见 ``_warn_multi_worker``.

用法 (业务方):
    # 1. 在 pipeline 里更新 phase 状态
    from forecast.task import set_phase, TaskStatus
    set_phase(task_id, "forecast", status=TaskStatus.processing, progress="500/10000")
    ...
    set_phase(task_id, "forecast", status=TaskStatus.success, result=results)

    # 2. 在 route 里挂任务
    from forecast.task import create_task, get_task, TaskID, TaskResult
...
async def create_task(
    ...
    **kwargs,
    ...
    调用方 ``fn`` 内部用 ``set_phase(task_id, ...)`` 上报进度.
    M2M 调用方拿到 ``task_id`` 后用 ``GET .../tasks/{task_id}`` 轮询取结果.
    """
    task_id = uuid4().hex
